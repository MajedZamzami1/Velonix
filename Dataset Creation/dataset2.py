...
def main():
    ...
    # After chunks_with_metadata is populated, create the CSV
    # Prepare data for CSV
    csv_data = {
        'Chunktext': [chunk['ChunkContent'] for chunk in chunks_with_metadata],
        'embedding': [chunk['Embedding'] for chunk in chunks_with_metadata],
        'Finance': [''] * len(chunks_with_metadata) 
    }

    # Create DataFrame and save to CSV
    df = pd.DataFrame(csv_data)
    csv_filename = f"{doc_name}_chunks.csv"
    df.to_csv(csv_filename, index=False)
    print(f"\nChunks saved to CSV: {csv_filename}")
